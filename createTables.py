...
session.execute('create table orderline ( w_id int, d_id int, o_id int, ol_number int, o_c_id int, o_ol_cnt decimal, o_all_local decimal, o_entry_d timestamp, ol_i_id int, ol_supply_w_id int, ol_delivery_d timestamp, ol_amount decimal, ol_quantity decimal, ol_dist_info text, PRIMARY KEY((w_id,d_id),o_id, ol_number)) with clustering order by (o_id desc, ol_number asc);')
...
session.execute('create table wd_payment ( w_id int, d_id int, w_ytd counter, d_ytd counter, primary key((w_id),d_id));')
...
session.execute('create table c_payment ( w_id int, d_id int, c_id int, c_balance counter, c_ytd_payment counter, c_payment_cnt counter, c_delivery_cnt counter, primary key((w_id,d_id),c_id));')
